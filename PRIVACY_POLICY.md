...
The extension stores only user preferences (such as keyboard shortcuts and display settings) locally on your device using Chrome's built-in storage API (`chrome.storage.sync`). This data:
...
| Permission | Purpose |
|------------|---------|
| `clipboardWrite` | To copy selected table data to your clipboard |
| `storage` | To save your preferences (shortcuts, display settings) |
| `activeTab` | To interact with the current tab's table content |
| `scripting` | To inject the selection functionality into web pages |
| `offscreen` | To handle clipboard operations in the background |
| `<all_urls>` | To work on any website with tables |
